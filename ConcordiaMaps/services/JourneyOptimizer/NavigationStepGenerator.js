...
export function generateSameBuildingSteps(from, to, steps) {
  console.log(
    `Generating steps within building ${from.buildingId}: ${from.room || "entrance"} → ${to.room || "exit"}`,
  );
  ...
  console.log(
    `Added step: Navigate within ${from.buildingId.toUpperCase()} building`,
  );
}
...
export function generateIndoorConnectedBuildingSteps(from, to, steps) {
  ...
  console.log(
    `Added tunnel navigation steps from ${from.buildingId.toUpperCase()} to ${to.buildingId.toUpperCase()}`,
  );
}
...
export function generateBuildingToBuildingOutdoorSteps(from, to, steps) {
  console.log(
    `Generating outdoor steps between buildings: ${from.buildingId} → ${to.buildingId}`,
  );
  ...
  console.log(
    `Added outdoor navigation steps from ${from.buildingId.toUpperCase()} to ${to.buildingId.toUpperCase()}`,
  );
}
...
export function generateBuildingToOutdoorSteps(from, to, steps) {
  console.log(
    `Generating building to outdoor steps: ${from.buildingId} → outdoor location`,
  );
  ...
  console.log(
    `Added building to outdoor navigation steps from ${from.buildingId.toUpperCase()}`,
  );
}
...
export function generateOutdoorToBuildingSteps(from, to, steps) {
  console.log(
    `Generating outdoor to building steps: outdoor location → ${to.buildingId}`,
  );
  ...
  console.log(
    `Added outdoor to building navigation steps to ${to.buildingId.toUpperCase()}`,
  );
}
...
export function generateOutdoorToOutdoorSteps(from, to, steps) {
  console.log(
    `Generating outdoor to outdoor steps: (${from.latitude}, ${from.longitude}) → (${to.latitude}, ${to.longitude})`,
  );
  ...
  for (let i = 0; i < optimizedLocations.length - 1; i++) {
    ...
    if (fromLocation.buildingId && toLocation.buildingId) {
      if (fromLocation.buildingId === toLocation.buildingId) {
        ...
        hasIndoorConnection(fromLocation.buildingId, toLocation.buildingId)
      ) {
