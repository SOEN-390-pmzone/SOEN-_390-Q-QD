...
export const RoomToRoomSameFloor = (baseCalculation) => (locationA, locationB) => {
    ...
    if (!floorGraph) {
      console.error(`Graph not found for building: ${buildingType}, floor: ${locationA.floor}`);
      return baseCalculation(locationA, locationB); // Fallback to base calculation
    }
    ...
    if (!path || path.length === 0) {
      console.error(`No path found between ${locationA.room} and ${locationB.room}`);
      return baseCalculation(locationA, locationB); // Fallback to base calculation
    }
    ...
    for (let i = 0; i < path.length - 1; i++) {
      ...
      totalDistance += floorGraph[currentNode][nextNode] * pathLengthToMetersRatio;
    }
