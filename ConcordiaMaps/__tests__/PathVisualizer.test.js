...
describe('PathVisualizer', () => {
  ...
  test('handles empty path', () => {
    ...
    expect(mockSvgElement.querySelectorAll).toHaveBeenCalledWith('.navigation-path');
  });
  ...
  test('creates path with correct attributes', () => {
    ...
    expect(mockPathElement.setAttribute).toHaveBeenCalledWith('stroke', '#3498db');
    expect(mockPathElement.setAttribute).toHaveBeenCalledWith('stroke-width', '5');
    expect(mockPathElement.classList.add).toHaveBeenCalledWith('navigation-path');
  });
}); 