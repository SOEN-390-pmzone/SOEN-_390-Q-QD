...
jest.mock('expo-calendar', () => ({
  ...
  getEventsAsync: jest.fn().mockResolvedValue([
    { id: '1', title: 'Event 1', startDate: new Date(), endDate: new Date(), notes: 'Room 101' },
  ]),
}));
...
describe('CalendarScreen', () => {
  it('selects other calendars', async () => {
    ...
    const selectBox = getByTestId('calendarButton');
  ...
  it('clicks on the Get Directions button and closes the alert', async () => {
    ...
    await waitFor(() => {
      expect(getByText('Get directions to Room 101')).toBeTruthy();
    });
  });
});