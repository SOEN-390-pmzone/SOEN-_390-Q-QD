...
jest.mock("../../../components/MultistepNavigation/DirectionArrow", () => "DirectionArrow");
...
describe("MultistepNavigationScreen Component", () => {
  ...
  it("renders correctly with default steps", () => {
    ...
    expect(getByText("Plan your journey across multiple buildings")).toBeTruthy();
    
    // Check if the description is rendered
    expect(getByText(/This feature allows you to navigate between different buildings/)).toBeTruthy();
    
    // Check if all default steps are rendered (there should be 5 steps)
    const stepTitles = getAllByTestId("step-title").map(node => node.props.children);
  ...
  it("handles type fallback correctly for bridge buildings", () => {
    ...
    const stepTypes = getAllByTestId("step-type").map(node => node.props.children);
