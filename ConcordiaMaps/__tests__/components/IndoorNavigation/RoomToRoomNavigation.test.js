...
import { handleSameFloorNavigation } from "../../../services/PathCalculationService";
...
describe("RoomToRoomNavigation Component", () => {
  ...
  it("initializes correctly with partial route params", async () => {
    ...
    await waitFor(() => {
      expect(findBuildingTypeFromId).toHaveBeenCalledWith(
        "hall",
        FloorRegistry,
      );
      expect(findFloorForRoom).toHaveBeenCalledWith(
        "HallBuilding",
        "H801",
        FloorRegistry,
      );
