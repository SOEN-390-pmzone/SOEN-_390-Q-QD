...
describe('IndoorNavigation Component', () => {
  ...
  const mockSvgContent = '<svg width="1024" height="1024"><rect x="10" y="10" width="80" height="80" fill="blue" /></svg>';
  ...
  it('renders correctly with building and floor information', async () => {
    ...
    expect(FloorRegistry.getFloorPlan).toHaveBeenCalledWith('HallBuilding', '8');
  });
  ...
  it('shows initial message before path calculation', async () => {
    ...
    await waitFor(() => {
      expect(getByText("Press 'Find Path' to calculate the route")).toBeTruthy();
    });
  });
  ...
  it('handles errors when loading floor plan', async () => {
    ...
    FloorRegistry.getFloorPlan.mockRejectedValue(new Error('Failed to load floor plan'));
