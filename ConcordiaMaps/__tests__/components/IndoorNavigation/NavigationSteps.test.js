...
describe('NavigationSteps', () => {
  ...
  it('renders correctly with steps', () => {
    ...
    expect(getByText('Start at room H801 on floor 8 of Hall Building')).toBeTruthy();
  ...
  it('provides accessibility labels for step items', () => {
    ...
    expect(getAllByLabelText('Start at room H801 on floor 8 of Hall Building').length).toBeGreaterThan(0);
    expect(getAllByLabelText('Go to H803').length).toBeGreaterThan(0);
    expect(getAllByLabelText('Take elevator to floor 9').length).toBeGreaterThan(0);
    expect(getAllByLabelText('Go to H903').length).toBeGreaterThan(0);
    expect(getAllByLabelText('Arrive at destination: H905').length).toBeGreaterThan(0);
  });
});