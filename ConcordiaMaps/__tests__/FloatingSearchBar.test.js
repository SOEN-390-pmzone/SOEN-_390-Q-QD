...
describe('FloatingSearchBar', () => {
  ...
  it('renders and updates search query', async () => {
    const { getByPlaceholderText } = render(<FloatingSearchBar onPlaceSelect={() => {}} />);
  ...
  it('fetches predictions and clears input', async () => {
    const mockPredictions = [{ place_id: '1', description: 'Montreal, QC, Canada' }];
    fetch.mockResolvedValueOnce({ json: () => Promise.resolve({ predictions: mockPredictions }) });

    const { getByPlaceholderText, getByText, queryByText } = render(<FloatingSearchBar />);
    const input = getByPlaceholderText('Search for a place...');

    await act(() => fireEvent.changeText(input, 'Mont'));
    expect(fetch).toHaveBeenCalledWith(expect.stringContaining('autocomplete/json'));
    ...
    it('does not fetch predictions when search query is less than 3 characters', async() => {
        ...
        await act(() => { fireEvent.changeText(input, "ab")});
        expect(fetch).not.toHaveBeenCalled();
    });

  it('calls onPlaceSelect with coordinates', async () => {
    const mockPredictions = [{ place_id: '1', description: 'Montreal, QC, Canada' }];
    const mockDetails = { result: { geometry: { location: { lat: 45.5, lng: -73.6 } } } };

    fetch.mockResolvedValueOnce({ json: () => Promise.resolve({ predictions: mockPredictions }) })
         .mockResolvedValueOnce({ json: () => Promise.resolve(mockDetails) });

    const onPlaceSelectMock = jest.fn();
    const { getByPlaceholderText, getByText } = render(<FloatingSearchBar onPlaceSelect={onPlaceSelectMock} />);

    await act(() => fireEvent.changeText(getByPlaceholderText('Search for a place...'), 'Montreal'));
    ...
    await waitFor(() => expect(onPlaceSelectMock).toHaveBeenCalledWith({ latitude: 45.5, longitude: -73.6 }));
  });
    ...
    it('handles fetch errors gracefully', async () => {
        ...
        await act(() => fireEvent.changeText(getByPlaceholderText('Search for a place...'), 'Error'));
