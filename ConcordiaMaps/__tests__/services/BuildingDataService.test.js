...
describe("Additional tests for remaining branches", () => {
  ...
  describe("isValidRoom", () => {
    test("should return false for null or undefined inputs", () => {
      expect(FloorRegistry.isValidRoom(null, "H-101")).toBe(false);
      expect(FloorRegistry.isValidRoom("H", null)).toBe(false);
    });
    test("should return true for entrance aliases", () => {
      expect(FloorRegistry.isValidRoom("H", "lobby")).toBe(true);
    });
    test("should return true for common facilities", () => {
      expect(FloorRegistry.isValidRoom("H", "restroom")).toBe(true);
    });
    test("should validate room if exists in valid rooms", () => {
      expect(FloorRegistry.isValidRoom("H", "H-101")).toBe(true);
    });
    test("should validate room using variations", () => {
      expect(FloorRegistry.isValidRoom("H", "H101")).toBe(true);
      expect(FloorRegistry.isValidRoom("H", "101")).toBe(true);
    });
    test("should validate MB room formats", () => {
      expect(FloorRegistry.isValidRoom("MB", "1.101")).toBe(true);
      expect(FloorRegistry.isValidRoom("MB", "MB-1.101")).toBe(true);
      expect(FloorRegistry.isValidRoom("MB", "1-101")).toBe(true);
      expect(FloorRegistry.isValidRoom("MB", "MB-1-101")).toBe(true);
      expect(FloorRegistry.isValidRoom("MB", "S2.230")).toBe(true);
      expect(FloorRegistry.isValidRoom("MB", "MB-S2.230")).toBe(true);
    });
    test("should validate Hall building room formats", () => {
      expect(FloorRegistry.isValidRoom("H", "H920")).toBe(true);
      expect(FloorRegistry.isValidRoom("H", "H-920")).toBe(true);
      expect(FloorRegistry.isValidRoom("H", "920")).toBe(true);
    });
    test("should return false for invalid room format", () => {
      expect(FloorRegistry.isValidRoom("H", "invalidRoom")).toBe(false);
    });
  });

  describe("getCoordinatesForBuilding", () => {
    test("should return correct coordinates for valid building id", () => {
      expect(FloorRegistry.getCoordinatesForBuilding("H")).toEqual({
        latitude: 45.497092,
        longitude: -73.5788,
      });
    });
    test("should return null for invalid building id", () => {
      expect(FloorRegistry.getCoordinatesForBuilding("XYZ")).toBeNull();
    });
  });

  describe("getReadableBuildingName", () => {
    test("should return readable name if building found", () => {
      expect(FloorRegistry.getReadableBuildingName("H")).toBe("Hall Building");
    });
    test("should return buildingId if no matching building found", () => {
      expect(FloorRegistry.getReadableBuildingName("XYZ")).toBe("XYZ");
    });
  });

  describe("getRoomPlaceholder", () => {
    test("should return default placeholder if buildingId is falsy", () => {
      expect(FloorRegistry.getRoomPlaceholder("")).toBe("Enter room number");
    });
    test("should return correct placeholder for known building", () => {
      expect(FloorRegistry.getRoomPlaceholder("VE")).toBe(
        "Enter room number (e.g. 101 or stairs)",
      );
    });
    test("should return default for unknown building", () => {
      expect(FloorRegistry.getRoomPlaceholder("XYZ")).toBe("Enter room number");
    });
  });

  describe("getErrorMessageForRoom", () => {
    test("should return generic error if buildingId is falsy", () => {
      expect(FloorRegistry.getErrorMessageForRoom("", "Some Building")).toBe(
        "Room not found.",
      );
    });
    test("should return specific error for MB", () => {
      expect(FloorRegistry.getErrorMessageForRoom("MB", "JMSB")).toBe(
        "Room not found. Try a format like 1.293 or 1-293.",
      );
    });
    test("should return specific error for VE", () => {
      expect(
        FloorRegistry.getErrorMessageForRoom("VE", "Vanier Extension"),
      ).toBe('Room not found. Try a room number or "elevator"/"stairs".');
    });
    test("should return default error message for unknown building", () => {
      expect(FloorRegistry.getErrorMessageForRoom("XYZ", "Unknown")).toBe(
        "This room doesn't exist in Unknown",
      );
    });
  });

  describe("_findElevatorForBuilding", () => {
    test("should return null if buildingType or floor is missing", () => {
      expect(FloorRegistry._findElevatorForBuilding(null, "1")).toBeNull();
      expect(
        FloorRegistry._findElevatorForBuilding("HallBuilding", null),
      ).toBeNull();
    });
    test("should return null if floor data is not found", () => {
      expect(
        FloorRegistry._findElevatorForBuilding("HallBuilding", "NonExistent"),
      ).toBeNull();
    });
    test("should find elevator based on possible keys", () => {
      const customFloor = {
        rooms: { elevator: { someData: true } },
      };
      const getFloorSpy = jest
        .spyOn(FloorRegistry, "getFloor")
        .mockReturnValue(customFloor);
      const result = FloorRegistry._findElevatorForBuilding(
        "HallBuilding",
        "1",
      );
      expect(result).toEqual({
    ...
    test("should find elevator when key has different case", () => {
      const customFloor = {
        rooms: { Elevator: { someData: true } },
      };
      const getFloorSpy = jest
      ...
      expect(result.title).toBe("Elevator");
      getFloorSpy.mockRestore();
    });
    test("should fallback to stairs if no elevator found", () => {
      const customFloor = {
        rooms: { stairwell: { someData: true } },
      };
      const getFloorSpy = jest
        .spyOn(FloorRegistry, "getFloor")
        .mockReturnValue(customFloor);
      ...
      expect(result.title).toBe("Stairs");
      getFloorSpy.mockRestore();
    });
    test("should use first available room if no elevator or stairs", () => {
      const customFloor = {
        rooms: { room1: { someData: true } },
      };
      const getFloorSpy = jest
        .spyOn(FloorRegistry, "getFloor")
        .mockReturnValue(customFloor);
      ...
      expect(result.title).toBe("Virtual Elevator");
      expect(result.room).toBe("room1");
      getFloorSpy.mockRestore();
    });
    test("should log error and return null if no rooms exist", () => {
      const customFloor = {
        rooms: {},
      };
      const getFloorSpy = jest
  ...
  describe("hasTunnelConnection", () => {
    test("should return true for hall and jmsb in any order", () => {
      expect(FloorRegistry.hasTunnelConnection("H", "JMSB")).toBe(true);
      expect(FloorRegistry.hasTunnelConnection("jmsb", "hall")).toBe(true);
    });
    test("should return false for same building", () => {
      expect(FloorRegistry.hasTunnelConnection("H", "H")).toBe(false);
    });
    test("should return false if one building id is invalid", () => {
      expect(FloorRegistry.hasTunnelConnection("H", "XYZ")).toBe(false);
    });
    test("should return false if both building ids are invalid", () => {
      expect(FloorRegistry.hasTunnelConnection("XYZ", "ABC")).toBe(false);
    });
  });
});
